...
import { calculateDefaultVertexPosition } from "../utils/other";
...
export class GraphPainter extends Painter {
  ...
  drawVertices(vertices) {
    for (let index in vertices) {
      ...
      const vertexPosition = this.getPosition(vertexId, calculateDefaultVertexPosition(index, vertices.length, this.style.graphRadius));
  ...
  highlightVertices(vertices, subobject) {
    if (vertices.length === this.model.vertices.length && vertices.filter(vertex => vertex != 0 && vertex != 1).length === 0) {
      vertices = vertices.map((vertex, index) => vertex == 1 ? index : undefined).filter(vertex => vertex !== undefined);
    }
  ...
  highlightEdges(edges, subobject) {
    for (let edge of edges) {
      const edgeLineNode = this.mainGroup.find(`#edge-line-${edge.a}-${edge.b}`)[0] ?? this.mainGroup.find(`#edge-line-${edge.a}-${edge.b}`)[0];
      if (edgeLineNode) {
